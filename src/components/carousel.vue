...
onMounted(() => {
    ...
    getFiles(path).then((files) => {
        ...
        if (sort === 'nameIncrease') {
            return pictures.sort((a, b) => compare(a.name, b.name)).map((f) => f.url);
        }
        if (sort === 'nameDecrease') {
            return pictures.sort((a, b) => compare(b.name, a.name)).map((f) => f.url);
        }
        if (sort === 'dateIncrease') {
            return pictures.sort((a, b) => compare(a.updated, b.updated)).map((f) => f.url);
        }
        return pictures.sort((a, b) => compare(b.updated, a.updated)).map((f) => f.url);
    }).then((urls) => images.value = urls);
})
</script>
